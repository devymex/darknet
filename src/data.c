...
data load_data_detection(int n, char **paths, int m, int w, int h, int c,
		...
		float resize, float hue, float saturation, float exposure,
	...
	for (int i = 0; i < n; ++i) {
		...
		float norm_h =	(float)ori_h / crop_h;
		int min_w_h = fill_truth_detection(filename, boxes, truth_size, truth,
				classes, flip, norm_x, norm_y, norm_w, norm_h, w, h);
